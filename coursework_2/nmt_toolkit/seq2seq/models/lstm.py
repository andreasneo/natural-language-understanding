...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        When self.bidirectional is to True, two LSTMs are used. This, allows the network
        to have both forward (left-to-right) and backward (right-to-left) information about
        the input sequence. The final hidden states of each LSTM (final\_hidden\_states) are
        concatenated. The final cell states (final\_cell\_states) of each LSTM are also
        concatenated using the same function (combine\_directions).

        The cell state is the memory of an LSTM and the hidden state is the output of that cell.
        The cell state has interactions with gates, that allow information to be added, removed or
        returned. The previous hidden state is used by these gates to make decisions.
        The cell state is the core idea of an LSTM that allows it to access long-term information
        about the data.
        '''
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        The attention weights are calculated suing the softmax of the attention scores (attn\_scores).
        The attention context is calculated as the product (weighted average) of attention weights and
        the outputs of the encoder (source states). The result is "squeezed", to delete all the dimensions of size 1.

        A mask is applied to the attention scores before calculating the weights to ensure that sentences of different
        lengths are handled.
    ...
    def score(self, tgt_input, encoder_out):
        ...
        Luong et al. examined different attention models (global, local-m, local-p) with different
        alignment/scoring functions (location, dot, general, concat). In this implementation of seq2seq,
        the global attention with general scoring function was used. The attention scores are calculated
        by projecting the encoder outputs to a feed-forward layer, and then multiplying it with the target
        input. This matrix multiplication (torch.bmm) aligns the encoder and decoder representations.
        '''
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        self.final_projection = nn.Linear(hidden_size, len(dictionary)) #hidden_size 128 #len(dictionary) 3712
        ...
        if self.use_lexical_model:
            ...
            self.lexical_projection = nn.Linear(embed_dim, len(dictionary)) #embed_dim 64
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        for j in range(tgt_time_steps):
            ...
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    attn_transpose = step_attn_weights.transpose(1,0) # [15,1]
                    attn_transpose = attn_transpose.unsqueeze(2) # [15,1,1]
                    # src_embeddings [15, 1, 64]
                    af = torch.bmm(attn_transpose , src_embeddings) # [15, 1, 64]
        ...
        decoder_output = torch.cat(rnn_outputs, dim=0).view(tgt_time_steps, batch_size, self.hidden_size) # [x++, 1, 128]
        ...
        # Final projection
        decoder_output = self.final_projection(decoder_output) #[ 1, x++, 3712]
    #    print ("DECODER OUTPUT")
    #    print (decoder_output)
        if self.use_lexical_model:
            # __QUESTION: Incorporate the LEXICAL MODEL into the prediction of target tokens here

            lexical_output_cat =   torch.cat(lexical_contexts, dim=0).view(tgt_time_steps, batch_size, self.embed_dim) # [x++, 1, 64]
