...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			# Sum of the dot products of V (input word representation matrix) with one-hot encoding of vocabulary
			# and U (matrix of reccurent hidden layer) with the previous time state of s.
			s[t] = sigmoid(np.dot(self.V , make_onehot(x[t], self.vocab_size))+ np.dot(self.U,s[t-1]))
			# Dot product of s at current time state with W (output word representation matrix).
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			# One-hot encoding of desired output at time t minus predicted output at t
			d_out = (make_onehot(d[t], self.out_vocab_size) - y[t])
			# Element-wise product of d_out with the derivative of softmax (all-ones vector)
			d_out_1 = np.multiply(d_out,np.ones(d_out.shape,dtype = int))
			# Sum of outer products of d_out_1 and predicted hidden layer at time t
			self.deltaW += np.outer(d_out_1, s[t])

			# Derivative of sigmoid function
			d_sigmoid = np.multiply(s[t],(np.ones(s[t].shape,dtype = int)-s[t]))
			# Element-wise product of the sigmoid with the dot product of W transpose with d_out_1
			d_in = np.multiply(np.dot(self.W.T,d_out_1), d_sigmoid )
			# Sum of the outer products of d_in with x at time t
			self.deltaV += np.outer(d_in, make_onehot(x[t], self.vocab_size))

			# Sum of the outer products of d_in at time t with s at t-1
	...
	def acc_deltas_np(self, x, d, y, s):
		...
		# Make predictions in the final timestep
		final_step = len(x) - 1

		# Modified deltaW, deltaV, deltaU to take into account only the final step
		d_out = (make_onehot(d[0], self.out_vocab_size) - y[final_step])
		d_out_1 = np.multiply(d_out,np.ones(d_out.shape,dtype = int))
		self.deltaW = np.outer(d_out_1, s[final_step])


		d_sigmoid = np.multiply(s[final_step],(np.ones(s[final_step].shape,dtype = int)-s[final_step]))
		d_in = np.multiply(np.dot(self.W.T,d_out_1) , d_sigmoid )
		self.deltaV = np.outer(d_in, make_onehot(x[final_step], self.vocab_size))

		self.deltaU = np.outer(d_in,s[final_step-1])
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			# deltaW same as acc_deltas
			d_out = (make_onehot(d[t], self.out_vocab_size) - y[t])
			d_out_1 = np.multiply(d_out,np.ones(d_out.shape,dtype = int))
			self.deltaW += np.outer(d_out_1, s[t])

			# deltaV same as acc_deltas
			d_sigmoid = np.multiply(s[t],(np.ones(s[t].shape,dtype = int)-s[t]))
			d_in = np.multiply(np.dot(self.W.T,d_out_1), d_sigmoid )
			self.deltaV += np.outer(d_in, make_onehot(x[t], self.vocab_size))

			# deltaU same as acc_deltas
			self.deltaU += np.outer(d_in,s[t-1])
			...
			for step in range(steps):
				# Starting from 1
				step = step + 1
				# Updated sigmoid derivative for t timesteps
				d_sigmoid =  np.multiply(s[t - step ], (np.ones(s[t].shape,dtype = int)-s[t - step]))
				# Element-wise product of d_sigmoid with the dot product of U transpose with d_in
				d_in = np.multiply(np.dot(self.U.T,d_in) , d_sigmoid)
				# Sum of outer products of d_in with one-hot vector of x at t-step
				self.deltaV += np.outer(d_in, make_onehot(x[t-step], self.vocab_size))

				# Sum of the outer products of d_in with s
				self.deltaU += np.outer(d_in,s[t-step-1])
	...
	def acc_deltas_bptt_np(self, x, d, y, s, steps):
		...
		for step in range(steps):
			step = step + 1
			# Check if step is greater than the lenght of the sequence
			if (step<=final_step):
				d_sigmoid = np.multiply(s[final_step - step ],(np.ones(s[final_step].shape,dtype = int)-s[final_step - step]))
				d_in = np.multiply(np.dot(self.U.T,d_in) , d_sigmoid )
				self.deltaV += np.outer(d_in, make_onehot(x[final_step-step], self.vocab_size))

				self.deltaU += np.outer(d_in,s[final_step-step-1])
	...
	def compute_loss(self, x, d):
		...
		y, _ = self.predict(x)
		for t in range(len(x)):
			loss_1 =  -np.dot(make_onehot(d[t], self.vocab_size),(np.log(y[t])))
			loss = loss + loss_1
		return loss
	...
	def compute_loss_np(self, x, d):
		...
		final_step = len(x) - 1
		y, _ = self.predict(x)
		loss =  -np.dot(make_onehot(d[0], self.vocab_size),(np.log(y[final_step])))
		return loss
	...
	def compare_num_pred(self, x, d):
		...
		y, _ = self.predict(x)
		return_value = 0
		if (y[final_step][d[0]] > y[final_step][d[1]] ):
			return_value = 1
		return return_value
	...
	def compute_mean_loss(self, X, D):
		...
		for i,sentence in enumerate(X):
			y, _ = self.predict(sentence)
			loss = 0.
			for t in range(len(sentence)):
				loss +=  -np.dot(make_onehot(D[i][t], self.out_vocab_size),(np.log(y[t])))
				total_words += 1
			loss_sum += loss

		mean_loss = loss_sum/total_words
...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		# Load the test set or (full dev set)
		...
		r.train(X = X_train, D = D_train,X_dev = X_dev,D_dev = D_dev,learning_rate=lr,back_steps=lookback)

		mean_loss = r.compute_mean_loss(X_test, D_test)
		print ('Mean loss on wiki-test.txt: ' + str(mean_loss))

		adjusted_loss = adjust_loss(mean_loss,fraction_lost,q,mode='')
		unadjusted_loss = adjust_loss(mean_loss,fraction_lost,q,mode='basic')
	...
	if mode == "train-np":
		...
		r.train_np(X = X_train, D = D_train,X_dev = X_dev,D_dev = D_dev,learning_rate=lr,back_steps=lookback)


		# Loss and accuracy for test set. Same as in train_np
		loss_function = r.compute_loss_np
		loss_sum = len(D_test)
		test_loss = sum([loss_function(X_test[i], D_test[i]) for i in range(len(X_test))]) / loss_sum
		test_acc = sum([r.compute_acc_np(X_test[i], D_test[i]) for i in range(len(X_test))]) / len(X_test)

		print("Test Loss: %.06f" % test_loss)
		print("Test Accuracy: %.06f" % test_acc)
